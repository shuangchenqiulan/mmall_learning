...
public class FTPUtil {
    ...
    private boolean uploadFile(String romotePath,List<File> fileList) throws IOException {
        ...
        if(connectServer(this.ip,this.port,this.user,this.pwd)) {
            try {
                ...
                for(File filtItem : fileList) {
                    fis = new FileInputStream(filtItem);
                    /*
                    存储文件到对应的文件夹中
                     */
                    ftpClient.storeFile(filtItem.getName(),fis);
                }
            } catch (IOException e) {
    ...
    public FTPClient getFftpClient() {
        return ftpClient;
    }

    public void setFftpClient(FTPClient ftpClient) {
